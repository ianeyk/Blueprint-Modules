...
## 4.3.8 Min/Max/Average Values

The `MIN`, `MAX`, and `AVG` commands return a single value --- the
minimum, maximum, and average value of the elements in a query. You can
be as specific as you like with the FROM and WHERE clauses, and you will
receive only the `MIN`, `MAX`, or `AVG` of those values you selected.
...
## 4.3.9 Count Number of Entries

A similar query is to find the total number of entries that satisfy your
given criteria. For this, use the `COUNT` command in the same way as
`MIN`, `MAX`, and `AVG`.
