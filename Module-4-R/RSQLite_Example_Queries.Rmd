...
note of caution: SQL returns rows in arbitrary order; the 100 rows that
we get from this example are not necessarily the "first" 100 rows in the
database. To get top 100 values ranked in a certain order, use
...
## 4.3.8 Min/Max/Total Number/Average Values

The `MIN`, `MAX`, `COUNT`, and `AVG` commands return a single value ---
the minimum, maximum, total number, and average value of the elements in
a query, respectively. You can be as specific as you like with the FROM
and WHERE clauses, and you will receive only the `MIN`, `MAX`, `COUNT`,
or`AVG` of those values you selected.
...
```{r eval = FALSE}
dbExecute(conn, "drop table high_pm25")

dbExecute(conn, "
CREATE TABLE IF NOT EXISTS high_pm25
  AS 
SELECT * FROM ts_kv_hourly
WHERE key = 10
AND val > 1000
...
dbGetQuery(conn, "
SELECT * FROM high_pm25
")

dbGetQuery(conn, "
SELECT min(val), max(val) FROM high_pm25
")

dbGetQuery(conn, "
SELECT DISTINCT entity_id FROM high_pm25
")
```
